...
let restController = {
    ...
    Restaurant.findAndCountAll({ include: Category, where: whereQuery, offset: offset, limit: pageLimit })
      .then(result => {
        ...
        const data = result.rows.map(r => ({
          ...
          isLiked: req.user.LikedRestaurants.map(id => id.id).includes(r.id),
        }))
        // console.log('isFavorited', data )
        Category.findAll({
          raw: true,
          nest: true
        }).then(categories => {
  ...
  getRestaurant: (req, res) => {
    return Restaurant.findByPk(req.params.id, {
      ...
      const isLiked = restaurant.LikedUsers.map(id => id.id).includes(req.user.id)
